import Constants from 'expo-constants'
import React from 'react';
import {View, Button, Text, StyleSheet,Vibration} from 'react-native'
const styles = StyleSheet.create({
  appContainer: {
    flex: 1,
    alignItems: 'center',
    backgroundColor:'black'
  },
  count: {
    fontSize: 48,
    color:'white',
  },
  text:{
    fontSize:30,
    color:'green',
  },
  todoContainer: {
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  size:
  {
    alignItems: 'center',
    width:200,
    height:50,
  },
})

class Study extends React.Component {
  constructor() {
    super()
    this.state = {
      minutes: 24,
      seconds: 59,
      isStudy: true,
      isRunning:false,
      totalStudy:0
    }
  }

  startCounter = () => {
        // clearInterval(this.interval) 
        if(!this.state.isRunning)
        {
          this.setState({isRunning:true,})
          this.interval = setInterval(this.timer, 1000)
        }
}

  resetCounter = () => {
        clearInterval(this.interval)
        this.setState({
        minutes: 24,
        seconds: 59,
        isRunning:false,
        totalStudy:0,
        });
  }

  stopCounter = () => {
        this.setState({isRunning:false,})
        clearInterval(this.interval)
  }

  timer = () => {
    if(this.state.minutes === 0 && this.state.seconds ===0)
    {
      Vibration.vibrate(500,500,500)
      if(this.state.isStudy)
      {
        this.setState({
        minutes: 4,
        seconds: 59,
        isStudy: false,
        // totalStudy: this.state.totalStudy + 25,
        });
      }
      else
      {
        this.setState({
        minutes: 24,
        seconds: 59,
        isStudy: true,
        });
      }
    }
    else if(this.state.seconds === 0)
    {
      if(this.state.isStudy)
      {
        this.setState({
        totalStudy: this.state.totalStudy + 1,
        });
      }
      this.setState(prevState => ({
        minutes: prevState.minutes - 1,
        seconds: 59,
      }));
    }
    else
    {
      this.setState(prevState => ({
        seconds: prevState.seconds - 1,
          }));
    }
  }

  render() {
    return (
      <View style={styles.appContainer}>
        <View style = {[styles.todoContainer,styles.size]}>
          <Button color="#00A000" title = "Start" onPress={this.startCounter} />
          <Button title="Reset" onPress={this.resetCounter} />
          <Button color="#FA0000" title="Stop" onPress={this.stopCounter} />
        </View>
        <Text style={styles.count}>{this.state.minutes}:{this.state.seconds} </Text>
        <Text style={styles.text}>Total Work: {this.state.totalStudy} minutes</Text>
      </View>
    )
  }
}

export default class App extends React.Component {
  constructor(props) {
    super(props)
    this.state = {}
  }
  render() {
    return (
      <View style={[styles.appContainer,styles.todoContainer]}>
        {<Study />}
      </View>
    )
  }
}